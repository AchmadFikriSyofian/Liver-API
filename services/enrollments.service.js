  ...
  const course = await courseRepository.getById ({id: course_id});
    ...
    if (course.type === 'isPremium') {
      if (metode_pembayaran === 'creditCard') {
        const creditCard = await creditCardRepository.create ({card_number, card_name, cvv, expired, user_id});
        const price = Number(course.price) + Number(course.price) * 0.11;

        const enrollment = await enrollmentRepository.create({price, metode_pembayaran, user_id, course_id})
